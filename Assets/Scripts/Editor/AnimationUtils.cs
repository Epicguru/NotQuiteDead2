...
public static class AnimationUtils
{
    ...
    public static void CreateOverrides()
    {
        ...
        foreach (var path in paths)
        {
            ...
            EditorUtility.DisplayProgressBar("Creating clips...", "Creating '{0}', {1}/{2}".Form(name, created + 1, total), (float)created / total);
        ...
        Debug.Log("Created {0} new animation clips...".Form(created));
        ...
        foreach (var clip in overrides)
        {
            if(clip.Value != null)
            {
                if (clips.ContainsKey(clip.Key.name))
                {
                    ...
                    Debug.LogWarning("There is already an override specified for '{0}', it will be replaced by the newly created clip. Old override was '{1}'".Form(clip.Key.name, oldPath));
        ...
        Debug.Log("Assigned {0}, replaced {1} and maintained {2} overrides".Form(added, replaced, maintained));
    }
